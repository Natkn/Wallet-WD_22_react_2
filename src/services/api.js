...
export const getTransactions = async (params = {}) => {
  try {
    ...
    throw new Error(error.response?.data?.error || 'Ошибка загрузки транзакций');
  }
};
...
export const updateTransaction = async (id, updateData) => {
  try {
    ...
    throw new Error(error.response?.data?.error || 'Ошибка обновления транзакции');
  }
};
...
export const deleteTransaction = async (id) => {
  try {
    ...
    throw new Error(error.response?.data?.error || 'Ошибка удаления транзакции');
  }
};
...
export const getTransactionsByPeriod = async (start, end) => {
  try {
    ...
    throw new Error(error.response?.data?.error || 'Ошибка получения данных за период');
  }
};
